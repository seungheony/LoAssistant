...
struct GetCrystalDataService {
    ...
    func getCrystalInfo(URL: String, completion : @escaping (NetworkResult<String>) -> Void) {
    ...
    private func judgeStatus(by statusCode: Int, _ data: Data) -> NetworkResult<String> {
    ...
    private func isValidData(data: Data) -> NetworkResult<String> {
        ...
        guard let decodedData = try? decoder.decode(CrystalModel.self, from: data) else { return .pathErr }
        
        // 성공적으로 decode를 마치면 success에다가 data 부분을 담아서 completion을 호출합니다.
        return .success(decodedData.buy)
    }
}
